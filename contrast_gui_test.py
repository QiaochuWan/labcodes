...
from PIL import Image, ImageTk, ImageOps  # Import ImageOps for resizing with aspect ratio
import numpy as np
...
class ImageApp:
    ...
    def load_image(self):
        ...
        if file_path:
            ...
            self.image_id = self.canvas.create_image(0, 0, image=self.photo, anchor=tk.NW)  # Display image on canvas
    ...
    def draw_line(self, event):
        ...
        if self.drawing_active:
            ...
            self.current_line_id = self.canvas.create_line(scaled_start, current_coords, fill="red" if len(self.line_coords) % 2 == 0 else "blue")
    ...
    def end_line(self, event):
        ...
        if self.drawing_active:
            ...
            label_id = self.canvas.create_text((scaled_start[0] + scaled_end[0]) // 2,
                                               ...
                                               text=f"{line_number}: {int(line_length)} px", fill="black")
    ...
    def zoom_image(self, event):
        ...
        if self.movement_enabled.get():
            ...
            for i, (start, end) in enumerate(self.original_line_coords):
                ...
                line_length = np.sqrt((scaled_end[0] - scaled_start[0]) ** 2 + 
                                    (scaled_end[1] - scaled_start[1]) ** 2)
                self.canvas.coords(self.line_labels[i],
                                (scaled_start[0] + scaled_end[0]) // 2,
                                (scaled_start[1] + scaled_end[1]) // 2)
                self.canvas.itemconfig(self.line_labels[i], text=f"{i + 1}: {int(line_length)} px")
